...
#define usb_debug_info(_fmt_, ...)   //printf("[scpi-INFO]:"_fmt_, ##__VA_ARGS__)	
	...
	/*VXI debug 0:disable	1:enable*/
#define VXI_DEBUG_ENABLE 1
	
#if (VXI_DEBUG_ENABLE)
		
#define vxi_debug(_fmt_, ...)        printf("[vxi] f:%s,l:%d,"_fmt_"\r\n",__FUNCTION__, __LINE__,##__VA_ARGS__)	
#define vxi_debug_warn(_fmt_, ...)   printf("[vxi-WARN] f%s,l:%d,"_fmt_"\r\n",__FUNCTION__, __LINE__,##__VA_ARGS__)	
#define vxi_debug_err(_fmt_, ...)    printf("[vxi-ERRO] f:%s,l:%d,"_fmt_"\r\n",__FUNCTION__, __LINE__,##__VA_ARGS__)	
#define vxi_debug_info(_fmt_, ...)   printf("[vxi-INFO] f:%s,l:%d,"_fmt_"\r\n",__FUNCTION__, __LINE__,##__VA_ARGS__)	
	
#else
	
#define vxi_debug(_fmt_, ...)       //printf("[scpi]:"_fmt_, ##__VA_ARGS__)	
#define vxi_debug_warn(_fmt_, ...)    do{
											printf("[vxi-WARN]:"_fmt_, ##__VA_ARGS__);
											syslog(LOG_WARNING, _fmt_",f:%s,l:%d", ##__VA_ARGS__, __FILE__, __LINE__);
											}while(0)
#define vxi_debug_err(_fmt_, ...)   do{
											printf("[vxi-ERRO]:"_fmt_, ##__VA_ARGS__);
											syslog(LOG_WARNING, _fmt_",f:%s,l:%d", ##__VA_ARGS__, __FILE__, __LINE__);
											}while(0)
#define vxi_debug_info(_fmt_, ...)   //printf("[scpi-INFO]:"_fmt_, ##__VA_ARGS__)	
#endif /*SCPI_DEBUG_ENABLE*/
