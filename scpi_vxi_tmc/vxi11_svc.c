...
#else
/*never return*/
int vxi11_srv_run(void)
{
	vxi_debug_info("vxi11 sever run!never return"); 
	svc_run ();
	vxi_debug_info ("svc_run returned");
	return -1;
...
int vxi11_init(void)
{
	...
	transp = svcudp_create(RPC_ANYSOCK);
	if (transp == NULL) {
		vxi_debug_err ("%s", "cannot create udp service.");
		return -1;
	}
	if (!svc_register(transp, DEVICE_ASYNC, DEVICE_ASYNC_VERSION, device_async_1, IPPROTO_UDP)) {
		vxi_debug_err ("%s", "unable to register (DEVICE_ASYNC, DEVICE_ASYNC_VERSION, udp).");
		return -1;
	}
	if (!svc_register(transp, DEVICE_CORE, DEVICE_CORE_VERSION, device_core_1, IPPROTO_UDP)) {
		vxi_debug_err ("%s", "unable to register (DEVICE_CORE, DEVICE_CORE_VERSION, udp).");
		return -1;
	}
	if (!svc_register(transp, DEVICE_INTR, DEVICE_INTR_VERSION, device_intr_1, IPPROTO_UDP)) {
		vxi_debug_err ("%s", "unable to register (DEVICE_INTR, DEVICE_INTR_VERSION, udp).");
		return -1;
	}
	vxi_debug("~svctcp_create..."); 
	transp = svctcp_create(RPC_ANYSOCK, 0, 0);
	if (transp == NULL) {
		vxi_debug_err ("%s", "cannot create tcp service.");
		return -1;
	}
	if (!svc_register(transp, DEVICE_ASYNC, DEVICE_ASYNC_VERSION, device_async_1, IPPROTO_TCP)) {
		vxi_debug_err ("%s", "unable to register (DEVICE_ASYNC, DEVICE_ASYNC_VERSION, tcp).");
		return -1;
	}
	if (!svc_register(transp, DEVICE_CORE, DEVICE_CORE_VERSION, device_core_1, IPPROTO_TCP)) {
		vxi_debug_err ("%s", "unable to register (DEVICE_CORE, DEVICE_CORE_VERSION, tcp).");
		return -1;
	}
	if (!svc_register(transp, DEVICE_INTR, DEVICE_INTR_VERSION, device_intr_1, IPPROTO_TCP)) {
		vxi_debug_err ("%s", "unable to register (DEVICE_INTR, DEVICE_INTR_VERSION, tcp).");
		return -1;
	}
